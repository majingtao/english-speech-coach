...
家长输入不会的单词 → 校验/自动生成词条 → 进「我的生词本」待学池 → 每天自动放 50 新 + 50 复习 → 孩子用 SRS 抽认卡背诵。
...
| 决策点 | 结论 |
|--------|------|
| 生词本与 SRS 关系 | **待学池模式**：加词只进生词本，每天从池中取 ≤50 入 SRS |
...
| 每日新词上限 50 | ✅ `DAILY_NEW_WORD_CAP = 50` | 同上 |
...
- **新词节奏**：加词只进生词本，不立刻进 SRS。每天首次进词汇页时，自动从**待学池里还没进 SRS 的词**取 `min(50 − 今日已入队, 剩余)` 个入队（`status=0`）。`DAILY_NEW_WORD_CAP=50` 现成。
- **复习节奏**：`today-review` 取到期词，上限 50。
- 池子剩余词第二天继续，不会一次压垮孩子。
...
### 验收标准
家长输入库内单词加入生词本 → 当日/次日自动进 ≤50 新词 → 孩子抽认卡背诵 → 记忆/遗忘驱动 SRS 复习。
